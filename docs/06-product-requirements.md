...
| PRD-FR-086 | P1 | Candidate | 用户可以用“听听回响”等自然语言入口查看全部记录、待办或提醒 |
| PRD-FR-087 | P1 | Candidate | 用户可以用“寻找回声”等自然语言入口搜索历史记录 |
| PRD-FR-088 | P1 | Candidate | “落幕”语义映射为完成，“归档”语义映射为归档，不与删除混淆 |
| PRD-FR-089 | P1 | Candidate | “让它远去”等删除表达必须进入目标确认后才执行删除 |
| PRD-FR-090 | P1 | Candidate | 用户可以用“和回响约定”等自然语言表达创建明确时间 Reminder |
| PRD-FR-091 | P1 | Candidate | 意图识别无法唯一确定目标或动作时，系统先澄清，不擅自修改数据 |
| PRD-FR-092 | P1 | Candidate | 回响式表达与直白表达映射到相同的底层 CRUD 和 Reminder 行为 |
| PRD-FR-093 | P1 | Candidate | 回响式反馈文案可以体现轻量仪式感，但不改写用户原始记录内容 |
| PRD-FR-094 | P1 | Candidate | 系统区分个人未来提醒时间与系统轮询、看门狗等自动化机制 |
| PRD-FR-095 | P1 | Candidate | 回响式自然语言交互纳入 v1.1 的兼容性、歧义和安全回归范围 |
