...
> V1.1 分层说明（2026-08-12 校准）：本章为**投递层** Target 状态机（scheduled/triggered/partially_completed，面向 V1.5 Delivery/Attempt 完整建模）。V1.1 已实现的是**用户回应层** Reminder 状态机（FR-002：pending/reminded/waiting_response/completed/archived/failed/cancelled，见 03-数据模型.md §5.9），两者分层并存、互不冲突；failed 终态标注由 V1.5 Delivery 聚合判定替代。
