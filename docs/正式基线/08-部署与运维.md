...
记录管理和 Schema upgrade 通过 `recall.py` 执行。常用命令：`list` / `get` / `search` / `add` / `update` / `done` / `delete`（留历史）/ `due` / `send-reminders` / `migrate` / `upgrade` / `restore` / `stats` / `view`；V1.1 起支持回响式自然语言入口 `talk`（记一下…/找…/完成…/还在等…回复/删掉…，删除类破坏性操作需确认，非交互环境自动拒绝），与直白命令映射到同一套 CRUD 行为。

### 2.4 先备份，再升级

涉及 Schema、Migration、批量修改和存储迁移时，必须先生成可验证备份。

### 2.4.1 Schema 迁移与恢复（V1.1）

- `recall.py upgrade`：Schema v1→v2 迁移——自动备份（`recall.backup-<时间戳>.json` 与 `recall_history` 备份）→ 预检（id 唯一）→ 迁移（schema_version→2.0；reminder_status `sent`→`reminded`）→ 自动校验（validate_recall.py）；
- `recall.py restore <备份文件>`：从备份恢复（迁移/故障回滚）；
- 备份文件保留在数据目录；迁移后可随时 restore 回滚。
