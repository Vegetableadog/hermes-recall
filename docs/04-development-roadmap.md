...
这些是候选范围。结构化字段不是已经冻结的最终 Schema；回响式表达不是已经实现的 Current 能力。
...
| 产品依据 | `06-product-requirements.md` 中的 `FEAT-V11-001`、`PRD-FR-085` 至 `PRD-FR-095`、`PRD-UX-014` 至 `PRD-UX-019` |
| 架构依据 | `02-system-architecture.md` 中 Target `Recall Intent Layer` |
| 数据影响 | 不新增第二事实来源；默认不改变 Recall 持久化 Schema |
| 兼容要求 | “记一下”“查看”“搜索”“完成”“删除”等现有直白表达继续可用 |
| 安全要求 | 删除、批量状态变更和多目标匹配必须经过明确确认；低置信度先澄清 |
| 测试要求 | 在 `05-test-plan.md` 建立 `V11-NLU` 专项回归后才进入实现 |
...
| 回归用例 | `05-test-plan.md` 中的 `V11-GW-001` |
...
- `01-project-overview.md` 已验收；
- `02-system-architecture.md` 已验收；
- `03-data-model.md` 已验收；
- `04-development-roadmap.md` 已验收；
- `05-test-plan.md` 已定义 v1.1 数据迁移和回归测试；
...
- v1.0 投递确认窗口有明确修复方案；
- `07-reminder-notification.md` 完成专项设计；
...
1. `06-product-requirements.md`：定义 `FEAT-V11-001` 产品行为与范围；
2. `02-system-architecture.md`：定义 Target `Recall Intent Layer`；
3. `04-development-roadmap.md`：定义 V1.1 候选范围、依赖和完成标准；
4. `05-test-plan.md`：定义 `V11-NLU` 与 `V11-GW-001` 回归和门禁。
